...
#define     BIT(x)				    ((unsigned int)((unsigned int)0x01U<<(x)))
// the mask of bits between start and end, including start and end
#define     BITS(start, end)		            ((0xFFFFFFFFUL << (start)) & (0xFFFFFFFFUL >> (31U - (unsigned int)(end))))
// the value of the x-th bit
#define     GET_BIT(regval, x)	                    (((regval) & BIT(x)) >> (x))
// the value of the bits between start and end, including start and end
...
unsigned int calc_crc24q_t(unsigned char* buff, unsigned int n_bits) {
    unsigned int i;
    unsigned int crc = 0;
    unsigned int n_byte = 0;
    unsigned int n_bit = 0;

    crc = (GET_BITS(*buff, 0, 7) << 16) | (GET_BITS(*(buff + 1), 0, 7) << 8) | GET_BITS(*(buff + 2), 0, 7);
    for (i = 24; i < n_bits; ++i) {
        n_byte = i / 8;
        n_bit = 7 - i % 8;
        crc = (crc << 1) | GET_BIT(*(buff + n_byte), n_bit);
        if (crc & 0x1000000) {
            crc = crc ^ POLYCRC24Q;
        }
        //crc = (crc << 1) & 0xffffff;
