                      ...
                      double *Lc, double *Pc)
{
    ...
    /* Iono-free LC (Linear Combination) */
    //GCC Add DCB correction
    *Lc=*Pc=0.0;
    double var = 0.0;
    if (opt->ionoopt == 3)
    {// Calculate the ionospheric-free combination coefficients
        aph = SQR(freq[0]) / (SQR(freq[0]) - SQR(freq[1]));
        beta = -SQR(freq[1]) / (SQR(freq[0]) - SQR(freq[1]));
        if (P[0] > 1000 && P[1] > 1000 && L[0] > 1000 && L[1] > 1000)
        {/* Complete dual-frequency data */
            // Calculate the ionospheric-free linear combination of phase measurements
            Lc[0] = aph * L[0] + beta * L[1];
            // Compute the ionospheric-free pseudo-range using dual-frequency data
            Pc[0] = prange_dualfrequency(obs, nav, &var, dants, dantr, opt); // Group the dual-frequency measurements
        }
        else
        {// Set ionospheric-free combination values to zero if the data is incomplete
            Lc[0] = 0.0;
            Pc[0] = 0.0;
            }
    }
...
static void udclk_ppp(rtk_t *rtk)
{
    ...
    /* initialize every epoch for clock (ite noise) */
    for (i=0;i<NSYS;i++) {
   ...
   /* if (opt->tidecorr) {
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        for (j=0;j<2*NF(opt);j++) {
            ...
            /* variance */
            //GCC Height-angle weighting only
            ...
            //GCC Modify the BeiDou residual threshold
            if (!post && opt->maxinno > 0.0 && (fabs(v[nv]) > (opt->maxinno + (opt->nf == 1 ? 50.0 : 0.0) + ((sys == SYS_CMP || sys == SYS_GAL) ? 30.0 : 0.0))))																																			   //if (!post&&opt->maxinno>0.0&&fabs(v[nv])>opt->maxinno) {
            {
                exc[i] = 1;
