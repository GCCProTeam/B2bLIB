                     ...
                     double *var)
{
    ...
    *var=0.0;
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* dual-frequency */
        ...
        else if (sys==SYS_CMP) { /* B1-B3 *///GCC Modify Beidou frequency
            gamma = SQR(((obs->code[0] == CODE_L2I) ? FREQ1_CMP : FREQ1) / FREQ3_CMP);// FREQ2_CMP);
                   ...
                   double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        //GCC Remove vare[i]
        var[nv++] = varerr(opt, azel[1 + i * 2], sys) + vmeas + vion + vtrp;
                  ...
                  char *msg)
{
    ...
    int i,stat,vsat[MAXOBS]={0},svh[MAXOBS], sateph;
    ...
    //GCC
    sateph = opt_.sateph;
    if (opt_.sateph = 5) sateph = 0;
    /* satellite positons, velocities and clocks */
    satposs(sol->time,obs,n,nav,sateph,rs,dts,var,svh);
